...
exports.authenticate = function(options,callback) { // user_key comes from the query or header, method comes from path matching with hardcoded values
    var URL ='https://su1.3scale.net/transactions/authrep.xml?';
    URL += "provider_key="+providerKey;
    URL += "&service_id=" + serviceID;

    if(options.user_key)
      URL += "&user_key="+ options.user_key;

    if(options.usage){
      Object.keys(options.usage).forEach(function(method){
        URL += "&usage"+"%5B"+method+"%5D="+options.usage[method];
      })
    }
